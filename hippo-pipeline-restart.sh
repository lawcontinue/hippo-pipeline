...
# 用法: ./hippo-pipeline-restart.sh [--r1-only | --r0-only] [--prompt "xxx"] [--max-tokens 200] [--rp 1.2] [--temp 0.0]
#
# 默认：重启 R1 并启动 R0 推理
# --r1-only: 只重启 R1
...
while [[ $# -gt 0 ]]; do
  case $1 in
    --r1-only) R1_ONLY=true; shift ;;
    --r0-only) R0_ONLY=true; shift ;;
    --prompt) PROMPT="$2"; shift 2 ;;
    --max-tokens) MAX_TOKENS="$2"; shift 2 ;;
    --rp) RP="$2"; shift 2 ;;
    --temp) TEMP="$2"; shift 2 ;;
    --port) R1_PORT="$2"; shift 2 ;;
